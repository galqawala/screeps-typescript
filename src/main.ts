...
declare global {
  ...
  type Action = 'build' | 'harvest' | 'moveTo' | 'pickup' | 'repair' | 'reserveController' | 'transfer' | 'upgradeController' | 'withdraw';
...
function isSpawnOrExtension(structure: Structure | null | undefined): structure is (StructureSpawn | StructureExtension) {
...
function handleHarvester(creep: Creep) {
  ...
  if (!isEmpty(creep)) {
    ...
    let myTarget = creep.pos.findClosestByPath(creep.pos.findInRange(FIND_MY_STRUCTURES, 3).filter(target => target.my !== false && target.hits < target.hitsMax));
    if (myTarget) creep.repair(myTarget);
    //repair unowned structures
    let target = creep.pos.findClosestByPath(creep.pos.findInRange(FIND_STRUCTURES, 3).filter(target => !isOwnedStructure(target) && target.hits < target.hitsMax));
...
function unloadCreep(creep: Creep) {
  ...
  let targetCreep = pos.findClosestByPath( //carrier
    pos.findInRange(FIND_CREEPS, 1).filter(target =>
      !isFull(target) && target.my !== false && (target.memory.role === 'carrier' || target.memory.role === 'spawner' || target.memory.role === 'worker')
    )
  );
  if (targetCreep) { creep.transfer(targetCreep, RESOURCE_ENERGY); return; }
  let myStructure = pos.findClosestByPath( //my structure
    pos.findInRange(FIND_MY_STRUCTURES, 1).filter(target => !isFull(target) && (target.my !== false))
  );
  if (myStructure) { creep.transfer(myStructure, RESOURCE_ENERGY); return; }
  let structure = pos.findClosestByPath( //unowned structure
    pos.findInRange(FIND_STRUCTURES, 1).filter(target => !isFull(target) && !isOwnedStructure(target))
  );
  if (structure) { creep.transfer(structure, RESOURCE_ENERGY); return; }
}
...
function bodyPartToAddByRatio(ratios: Partial<Record<BodyPartConstant, number>>, partAmounts: Partial<Record<BodyPartConstant, number>>) {
...
function handleRoom(room: Room) {
  //control the towers
  let towers = <StructureTower[]>room.find(FIND_MY_STRUCTURES).filter(tower => tower.structureType === STRUCTURE_TOWER);
  for (const t of towers) { handleTower(t); }
...
function handleHostilesInRoom(room: Room) {
  ...
  if (room.memory.hostilesPresent !== hostilesPresent) {
    if (hostilesPresent) {
      let hostileOwners = hostileCreeps.map(creep => creep.owner.username).concat(hostilePowerCreeps.map(creep => creep.owner.username)).filter(onlyUnique);
  ...
  if (hostilesPresent) {
    let towerCount = room.find(FIND_MY_STRUCTURES).filter(tower => tower.structureType === STRUCTURE_TOWER).length;
    if (towerCount <= 0) {
...
function blockedByStructure(pos: RoomPosition) {
  return pos.lookFor(LOOK_STRUCTURES).filter(structure => (OBSTACLE_OBJECT_TYPES as StructureConstant[]).includes(structure.structureType)).length > 0;
}

function containsPosition(list: RoomPosition[], pos: RoomPosition) {
  return list.filter(listPos => listPos.x === pos.x && listPos.y === pos.y && listPos.roomName === pos.roomName).length > 0;
}
...
function handleLinks(room: Room) {
  ...
  let links = room.find(FIND_MY_STRUCTURES).filter(isLink)
...
function handleTower(tower: StructureTower) {
  let bestTarget; let bestTargetScore = Number.NEGATIVE_INFINITY;
  let creeps = tower.room.find(FIND_CREEPS).filter(target => target.my === false || target.hits < (target.hitsMax / 2));
  ...
  let structures = tower.room.find(FIND_STRUCTURES).filter(target => !isOwnedStructure(target) || target.hits < (target.hitsMax / 2));
  ...
  let powerCreeps = tower.room.find(FIND_POWER_CREEPS).filter(target => target.my === false || target.hits < (target.hitsMax / 2));
...
function getDestinationFromMemory(creep: Creep) {
  ...
  if ((!(creep.memory.empty) && isEmpty(creep)) || (!(creep.memory.full) && isFull(creep))) {
    ...
    if (creep.memory.action === 'repair' && destination && 'hits' in destination && destination instanceof Structure && !needsRepair(destination)) {
...
function getEnergyDestinations() {
  ...
  for (const i in Game.rooms) {
    ...
    let roomTargets = room.find(FIND_MY_STRUCTURES).filter(structure => structure.structureType === STRUCTURE_TOWER && !isFull(structure));
...
function getEnergySources(myMinTransfer: number, allowStorage = false, allowAnyLink = false, allowSource = false) {
  let sources: any[] = [];
...
function getEnergyInRoom(room: Room, myMinTransfer: Number, pos: RoomPosition, allowStorage = true, allowAnyLink = true, allowSource = true) {
  let sources: any[] = room.find(FIND_DROPPED_RESOURCES).filter(resource => getEnergy(resource) >= myMinTransfer);
  ...
  sources = sources.concat(room.find(FIND_STRUCTURES).filter(structure =>
    (structure.structureType === STRUCTURE_CONTAINER
      || (structure.structureType === STRUCTURE_STORAGE && allowStorage)
      || (isLink(structure) && allowAnyLink)
      || isDownstreamLink(structure)
...
function action(creep: Creep, destination: Destination) {
  ...
  } else if (creep.memory.action === 'withdraw' && (destination instanceof Structure || destination instanceof Tombstone || destination instanceof Ruin)) {
  ...
  } else if (creep.memory.action === 'transfer' && (destination instanceof Creep || destination instanceof Structure)) {
  ...
  } else if (creep.memory.action === 'harvest' && (destination instanceof Source || destination instanceof Mineral || destination instanceof Deposit)) {
...
function postAction(creep: Creep, destination: Destination, actionOutcome: ScreepsReturnCode) {
  if (actionOutcome === OK) {
    ...
    if (actionOutcome === ERR_NOT_IN_RANGE && (destination instanceof RoomPosition || 'pos' in destination)) {
      ...
      creep.say('🔎');
      resetDestination(creep);
      if (destination instanceof Structure || destination instanceof RoomPosition) memorizeBlockedObject(creep, destination);
...
function getRepairTaskInRange(pos: RoomPosition) {
  let destination = pos.findClosestByPath(
    pos.findInRange(FIND_MY_STRUCTURES, 3).filter(target => target.my !== false && target.hits < target.hitsMax)
  );
  if (destination) return { action: 'repair', destination: destination };
  let unowned = pos.findClosestByPath(
    pos.findInRange(FIND_STRUCTURES, 3).filter(target => !isOwnedStructure(target) && target.hits < target.hitsMax)
  );
...
function getRepairTask(creep: Creep) {
  ...
  for (const i in Game.rooms) {
    ...
    destinations = destinations
      .concat(room.find(FIND_STRUCTURES).filter(target => worthRepair(creep.pos, target) && !isUnderRepair(target) && !isBlocked(creep, target)));
  }
  ...
  if (task) {
    if ('id' in task.destination) {
      ...
      if (destinationRoomName) {
        if (task.destination instanceof RoomPosition) Memory.rooms[destinationRoomName].lastEnergyConsumerPos = task.destination;
        else if (task.destination.pos instanceof RoomPosition) Memory.rooms[destinationRoomName].lastEnergyConsumerPos = task.destination.pos;
      }
    }
    return task;
  }
...
function getConstructionSites(creep: Creep) {
  ...
  for (const i in Game.rooms) {
    let room = Game.rooms[i];
    sites = sites.concat(
      room.find(FIND_MY_CONSTRUCTION_SITES).filter(target => !isBlocked(creep, target))
    );
  }
  return sites;
}
...
function tryResetSpawnsAndExtensionsSorting(room: Room) {
  ...
  if (room.find(FIND_MY_STRUCTURES).filter(structure =>
...
function getPosForStorage(room: Room) {
  ...
  let targetPos;
  let link = controller.pos.findClosestByRange(FIND_MY_STRUCTURES, { filter: { structureType: STRUCTURE_LINK } });
  if (link) {
    targetPos = link.pos;
  } else {
    let site = controller.pos.findClosestByRange(FIND_MY_CONSTRUCTION_SITES, { filter: { structureType: STRUCTURE_LINK } });
...
function hasStructureInRange(pos: RoomPosition, structureType: StructureConstant | undefined, range: number, includeConstructionSites: boolean) {
  if (pos.findInRange(FIND_MY_STRUCTURES, range).filter(structure => (!structureType || structure.structureType === structureType)).length > 0) return true;

  if (includeConstructionSites && pos.findInRange(FIND_MY_CONSTRUCTION_SITES, range)
    .filter(structure => (!structureType || structure.structureType === structureType)).length > 0) return true;
...
function adjustConstructionSiteScoreForLink(score: number, pos: RoomPosition) {
  ...
  let linkSite = pos.findClosestByRange(FIND_MY_CONSTRUCTION_SITES, { filter: { structureType: STRUCTURE_LINK } });
  if (linkSite) {
...
function getSpawnsAndExtensionsSorted(room: Room) {
  //First filled spawns/extensions should be used first, as they are probably easier to refill
  let all = room.find(FIND_MY_STRUCTURES).filter(structure => (structure.structureType === STRUCTURE_EXTENSION || structure.structureType === STRUCTURE_SPAWN));

  return room.memory.sortedSpawnStructureIds.map(id => Game.getObjectById(id)).concat(shuffle(all)).filter(onlyUnique).filter(isSpawnOrExtension);
}

function initialCreepMemory(role: Role, sourceId: undefined | Id<Source>, targetPos: undefined | RoomPosition, pos: RoomPosition) {
  return {
    role: role,
    targetPos: targetPos,
    sourceId: sourceId,
...
function spawnCreep(spawn: StructureSpawn, roleToSpawn: Role, energyAvailable: number, body: undefined | BodyPartConstant[]) {
  ...
  if (!body) {
    ...
    else if (roleToSpawn === 'carrier' || roleToSpawn === 'spawner') body = bodyByRatio({ move: 1, carry: 1 }, energyAvailable);
  ...
  if (body && spawn.spawnCreep(body, name, { memory: initialCreepMemory(roleToSpawn, undefined, undefined, spawn.pos), energyStructures: energyStructures }) === OK) {
    msg(spawn, 'Spawning: ' + roleToSpawn + ' (' + name + '), cost: '
...
function construct(room: Room, structureType: BuildableStructureConstant) {
  if (needStructure(room, structureType)) {
    ...
    if (structureType === STRUCTURE_LINK) {
      pos.findInRange(FIND_STRUCTURES, 1).filter(target => target.structureType === STRUCTURE_CONTAINER).forEach(structure => {
...
function countStructures(room: Room, structureType: StructureConstant, includeConstructionSites: boolean) {
  let count = room.find(FIND_MY_STRUCTURES).filter(structure => structure.structureType === structureType).length;
  if (includeConstructionSites) {
    count += room.find(FIND_MY_CONSTRUCTION_SITES).filter(structure => structure.structureType === structureType).length;
  }
  return count;
}
...
function resetDestination(creep: Creep) {
  ...
  let destination;
  if (!(creep.memory.destination instanceof RoomPosition)) destination = Game.getObjectById(creep.memory.destination);
